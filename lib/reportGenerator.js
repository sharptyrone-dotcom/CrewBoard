...
export function generateComplianceReport({ vesselName = 'M/Y Serenity', crew = [], notices = [], docs = [], activity = [], training = [], dateRange = {} } = {}) {
  ...
  for (const member of crew) {
    ...
    const totalItems = filteredNotices.length + requiredDocs.length + trainingTotal;
    const overall = pct(noticesRead + docsAcked + trainingCompleted, totalItems);
    ...
    doc.autoTable({
      ...
      didParseCell: (data) => {
        if (data.section !== 'body') return;
        // Highlight overdue count in red if > 0
        ...
        // Overall Compliance — bold + colour-code
        if (data.row.index === 5) {
          data.cell.styles.fontStyle = 'bold';
