...
// Maps notification metadata to the preference column. Returns null for
// types that should always be delivered (e.g. critical notices).
...
async function filterIdsByPreference(crewMemberIds, vesselId, preferenceColumn) {
  ...
  try {
    ...
    console.error('[sendCrewNotification] preference check failed (sending to all)', err);
...
//
// Params:
//   vesselId          — vessel scope for push-subscription + prefs lookup
...
//   priority          — 'critical' | 'important' | 'routine' (notices only)
//   notificationType  — optional override for preference-column selection
//                       (e.g. 'training_reminder', 'event_update')
