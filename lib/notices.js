...
// the existing CrewNotice UI expects: { id, title, body, category, priority,
...
// Bulk pin/unpin helper used by admin BulkActionBar. Updates multiple
// notices in a single request — RLS policy notices_update_admin enforces
// admin-only on the server.
export async function updateNoticesPinned({ noticeIds, pinned }) {
  const { error } = await supabase
    .from('notices')
    .update({ is_pinned: pinned })
    .in('id', noticeIds);
  if (error) {
    console.error('[notices] bulk pin failed', error);
    throw error;
  }
}

// Soft-archive by setting expires_at=now. The notice is kept for audit
// history but hidden from active views that filter on expires_at > now.
export async function archiveNotices({ noticeIds }) {
  ...
  const { error } = await supabase
    ...
    .in('id', noticeIds);
  if (error) {
    console.error('[notices] bulk archive failed', error);
    throw error;
  }
  return now;
}
