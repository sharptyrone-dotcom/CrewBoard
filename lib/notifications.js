...
// caller in components/CrewNotice.js passes the full signature.
...
// Inserts a targeted notification visible only to a single crew member.
// Used by the "Send Reminder" flows to nudge specific crew about unread
// notices or unacknowledged documents. The notification shows up only in
// that crew member's bell panel (the fetch query filters on target_crew_id).
export async function createTargetedNotification({ targetCrewId, type, title, body, referenceType, referenceId }) {
  if (!targetCrewId) throw new Error('targetCrewId is required for targeted notifications');
  const { data, error } = await supabase
    .from('notifications')
    .insert({
  ...
  if (error) {
    console.error('[notifications] targeted create failed', error);
    throw error;
  }
