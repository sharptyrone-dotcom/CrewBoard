...
export default function CrewBoard({ user }) {
  ...
  return (
    <div style={{ background: T.bg, color: T.text, minHeight: '100vh', maxWidth: isDesktop ? undefined : 480, margin: isDesktop ? undefined : '0 auto', position: 'relative', display: 'flex', flexDirection: 'column', boxShadow: isDesktop ? 'none' : '0 0 80px rgba(15,23,42,0.06)' }}>
      ...
      {showNewNotice && <NewNoticeModal newNotice={newNotice} setNewNotice={setNewNotice} setShowNewNotice={setShowNewNotice} handlePostNotice={handlePostNotice} />}
      {showNewDoc && <NewDocumentModal newDoc={newDoc} setNewDoc={setNewDoc} setShowNewDoc={setShowNewDoc} handleUploadDoc={handleUploadDoc} uploadingDoc={uploadingDoc} />}
